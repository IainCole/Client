#include "RundownAtemKeyerInputWidget.h"
...
RundownAtemKeyerInputWidget::RundownAtemKeyerInputWidget(const LibraryModel& model, QWidget* parent, const QString& color, bool active,
...
void RundownAtemKeyerInputWidget::preview(const PreviewEvent& event)
{
...
void RundownAtemKeyerInputWidget::labelChanged(const LabelChangedEvent& event)
{
    // This event is not for us.
    if (!this->active)
        return;
...
void RundownAtemKeyerInputWidget::atemDeviceChanged(const AtemDeviceChangedEvent& event)
{
    // This event is not for us.
    if (!this->active)
        return;
...
AbstractRundownWidget* RundownAtemKeyerInputWidget::clone()
{
    RundownAtemKeyerInputWidget* widget = new RundownAtemKeyerInputWidget(this->model, this->parentWidget(), this->color, this->active,
                                                                          this->inGroup, this->compactView);

    AtemKeyerInputCommand* command = dynamic_cast<AtemKeyerInputCommand*>(widget->getCommand());
...
void RundownAtemKeyerInputWidget::setCompactView(bool compactView)
{
    if (compactView)
    {
...
void RundownAtemKeyerInputWidget::readProperties(boost::property_tree::wptree& pt)
{
    if (pt.count(L"color") > 0) setColor(QString::fromStdWString(pt.get<std::wstring>(L"color")));
}

void RundownAtemKeyerInputWidget::writeProperties(QXmlStreamWriter* writer)
{
    writer->writeTextElement("color", this->color);
}

bool RundownAtemKeyerInputWidget::isGroup() const
{
    return false;
}

bool RundownAtemKeyerInputWidget::isInGroup() const
{
    return this->inGroup;
}

AbstractCommand* RundownAtemKeyerInputWidget::getCommand()
{
    return &this->command;
}

LibraryModel* RundownAtemKeyerInputWidget::getLibraryModel()
{
    return &this->model;
}

void RundownAtemKeyerInputWidget::setActive(bool active)
{
    this->active = active;
...
void RundownAtemKeyerInputWidget::setInGroup(bool inGroup)
{
    this->inGroup = inGroup;
    this->labelGroupColor->setVisible(this->inGroup);
}

QString RundownAtemKeyerInputWidget::getColor() const
{
    return this->color;
}

void RundownAtemKeyerInputWidget::setColor(const QString& color)
{
    this->color = color;
    this->setStyleSheet(QString("#frameItem, #frameStatus { background-color: %1; }").arg(color));
}

void RundownAtemKeyerInputWidget::checkEmptyDevice()
{
...
void RundownAtemKeyerInputWidget::clearDelayedCommands()
{
    this->executeTimer.stop();
}

void RundownAtemKeyerInputWidget::setUsed(bool used)
{
    if (used)
    {
...
bool RundownAtemKeyerInputWidget::executeCommand(Playout::PlayoutType::Type type)
{
...
void RundownAtemKeyerInputWidget::executePlay()
{
    const QSharedPointer<AtemDevice> device = AtemDeviceManager::getInstance().getDeviceByName(this->model.getDeviceName());
    if (device != NULL && device->isConnected())
        device->setKeyerInput(this->command.getKeyer(), this->command.getInput());
	
    if (this->markUsedItems)
        setUsed(true);
}

void RundownAtemKeyerInputWidget::delayChanged(int delay)
{
    this->labelDelay->setText(QString("Delay: %1").arg(delay));
}

void RundownAtemKeyerInputWidget::checkGpiConnection()
{
...
void RundownAtemKeyerInputWidget::checkDeviceConnection()
{
    const QSharedPointer<AtemDevice> device = AtemDeviceManager::getInstance().getDeviceByName(this->model.getDeviceName());
    if (device == NULL)
...
void RundownAtemKeyerInputWidget::configureOscSubscriptions()
{
...
void RundownAtemKeyerInputWidget::allowGpiChanged(bool allowGpi)
{
    checkGpiConnection();
}

void RundownAtemKeyerInputWidget::gpiConnectionStateChanged(bool connected, GpiDevice* device)
{
    checkGpiConnection();
}

void RundownAtemKeyerInputWidget::remoteTriggerIdChanged(const QString& remoteTriggerId)
{
    configureOscSubscriptions();
...
void RundownAtemKeyerInputWidget::deviceConnectionStateChanged(AtemDevice& device)
{
    checkDeviceConnection();
}

void RundownAtemKeyerInputWidget::deviceAdded(AtemDevice& device)
{
...
void RundownAtemKeyerInputWidget::playControlSubscriptionReceived(const QString& predicate, const QList<QVariant>& arguments)
{
    if (this->command.getAllowRemoteTriggering() && arguments.count() > 0 && arguments[0] == 1)
        executeCommand(Playout::PlayoutType::Play);
}

void RundownAtemKeyerInputWidget::playNowControlSubscriptionReceived(const QString& predicate, const QList<QVariant>& arguments)
{
    if (this->command.getAllowRemoteTriggering() && arguments.count() > 0 && arguments[0] == 1)
        executeCommand(Playout::PlayoutType::PlayNow);
}

void RundownAtemKeyerInputWidget::updateControlSubscriptionReceived(const QString& predicate, const QList<QVariant>& arguments)
{
    if (this->command.getAllowRemoteTriggering() && arguments.count() > 0 && arguments[0] == 1)
